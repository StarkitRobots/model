...
namespace Leph
{
HumanoidModel::HumanoidModel(RobotType type, const std::string& frameRoot, bool isFloatingBase,
                             const Eigen::MatrixXd& inertiaData, const std::map<std::string, size_t>& inertiaName,
                             const Eigen::MatrixXd& geometryData, const std::map<std::string, size_t>& geometryName)
  : Model()
{
  ...
  initialize(urdfFile, type, frameRoot, isFloatingBase, inertiaData, inertiaName, geometryData, geometryName);
}

HumanoidModel::HumanoidModel(const std::string& urdfFile, RobotType type, const std::string& frameRoot,
                             bool isFloatingBase, const Eigen::MatrixXd& inertiaData,
                             const std::map<std::string, size_t>& inertiaName, const Eigen::MatrixXd& geometryData,
...
void HumanoidModel::initialize(const std::string& urdfFile, RobotType type, const std::string& frameRoot,
                               bool isFloatingBase, const Eigen::MatrixXd& inertiaData,
                               const std::map<std::string, size_t>& inertiaName, const Eigen::MatrixXd& geometryData,
                               const std::map<std::string, size_t>& geometryName)
{
  ...
  if (inertiaData.rows() > 0 && (size_t)inertiaData.rows() == inertiaName.size())
  {
  ...
  if (geometryData.rows() > 0 && (size_t)geometryData.rows() == geometryName.size())
  {
  ...
  RBDL::Model modelOld;
  if (!RBDL::Addons::URDFReadFromFile(urdfFile.c_str(), &modelOld, false, &tmpInertiaData, &tmpInertiaName,
                                      isInertiaOverride, &tmpGeometryData, &tmpGeometryName, isGeometryOverride))
  {
  ...
  else
  {
    ...
    frameRootId = wrappedModelNew.frameIndexToBodyId(wrappedModelNew.getFrameIndex(frameRoot));
  }

  // Update old urdf model with new root frame
  RBDL::Model modelNew = Leph::RBDLRootUpdate(modelOld, frameRootId, isFloatingBase);
  // Initialize base model
  Model::initializeModel(modelNew, tmpInertiaData, tmpInertiaName, tmpGeometryData, tmpGeometryName);

  // Compute leg segments length
  Eigen::Vector3d hipPt = Model::position("right_hip_roll", "origin");
  Eigen::Vector3d kneePt = Model::position("right_knee", "origin");
  Eigen::Vector3d anklePt = Model::position("right_ankle_pitch", "origin");
  Eigen::Vector3d footPt = Model::position("right_foot_tip", "origin");
...
void HumanoidModel::boundingBox(size_t frameIndex, double& sizeX, double& sizeY, double& sizeZ,
                                Eigen::Vector3d& center) const
{
  if (_type == SigmabanModel)
  {
    ...
    {
      Model::boundingBox(frameIndex, sizeX, sizeY, sizeZ, center);
    }
  }
...
bool HumanoidModel::legIkLeft(const std::string& frame, const Eigen::Vector3d& footPos, const Eigen::Matrix3d& rotation,
                              double* boundIKDistance)
{
  ...
  LegIK::Vector3D legIKTarget = buildTargetPos(frame, footPos, true);
  // Convert orientation from given frame
  // to LegIK base
  LegIK::Frame3D legIKMatrix = buildTargetOrientation(frame, rotation);

  // Run inverse kinematics
  LegIK::Position result;
  bool isSucess = ik.compute(legIKTarget, legIKMatrix, result, boundIKDistance);
...
bool HumanoidModel::legIkRight(const std::string& frame, const Eigen::Vector3d& footPos,
                               const Eigen::Matrix3d& rotation, double* boundIKDistance)
{
  // LegIK initialization
  LegIK::IK ik(_legHipToKnee, _legKneeToAnkle, _legAnkleToGround);
  // Convert foot position from given
  // target to LegIK base
  LegIK::Vector3D legIKTarget = buildTargetPos(frame, footPos, false);
  // Convert orientation from given frame
  // to LegIK base
  LegIK::Frame3D legIKMatrix = buildTargetOrientation(frame, rotation);
...
Eigen::Vector3d HumanoidModel::getPose()
{
  ...
  return Eigen::Vector3d(pos.x(), pos.y(), orientationYaw("trunk", "origin"));
}
...
Eigen::Vector3d HumanoidModel::trunkSelfOrientation()
{
  ...
  // Pitch
  angles(1) = atan2(-mat(0, 2), sqrt(mat(0, 0) * mat(0, 0) + mat(0, 1) * mat(0, 1)));
  // Yaw
...
Eigen::Matrix3d HumanoidModel::selfFrameOrientation(const std::string& frame)
{
  ...
  double pitch = atan2(-originToTrunk(0, 2),
                       sqrt(originToTrunk(0, 0) * originToTrunk(0, 0) + originToTrunk(0, 1) * originToTrunk(0, 1)));
  ...
  Eigen::Matrix3d baseToFrame = rollRot.toRotationMatrix() * pitchRot.toRotationMatrix();
  // Then adding trunk to frame orientation
  baseToFrame = trunkToFrame * baseToFrame;

  return baseToFrame;
}
Eigen::Vector3d HumanoidModel::selfFramePosition(const std::string& frame)
{
  ...
  Eigen::Vector3d translationInBase =
      Eigen::AngleAxisd(-yaw, Eigen::Vector3d::UnitZ()).toRotationMatrix() * translationInOrigin;
...
Eigen::Vector3d HumanoidModel::selfInFrame(const std::string& name, const Eigen::Vector3d& pos)
{
  ...
  Eigen::Vector3d selfToFrameVect = mat * selfFramePosition(name);
...
Eigen::Vector3d HumanoidModel::frameInSelf(const std::string& name, const Eigen::Vector3d& pos)
{
...
Eigen::Vector3d HumanoidModel::cameraPixelToViewVector(const CameraModel& cameraModel, const Eigen::Vector2d& pixel)
{
...
Eigen::Vector3d HumanoidModel::cameraPanTiltToViewVector(const Eigen::Vector2d& anglesPanTilt)
{
  // World to Self yaw
...
bool HumanoidModel::cameraViewVectorToWorld(const Eigen::Vector3d& viewVector, Eigen::Vector3d& pos,
                                            double groundHeight)
{
  // Optical center
...
bool HumanoidModel::cameraViewVectorToBallWorld(
    const CameraModel& cameraModel, const Eigen::Vector3d& viewVector, double radius, Eigen::Vector3d& ballCenter,
    ...
    std::vector<Eigen::Vector3d>* borders)
{
  ...
  ballCenter = center + forward.normalized() * (opticalCenterToPointDist - ballCenterToPointDist);
...
bool HumanoidModel::getImgRadiusFromViewVector(const CameraModel& cameraModel, const Eigen::Vector3d& viewVector,
                                               double ballRadius, double* minRadiusImg, double* maxRadiusImg)
{
  Eigen::Vector3d ballCenter;
  Eigen::Vector2d ballCenterPixel;
  std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>> bordersPixel;
  bool success =
      cameraViewVectorToBallWorld(cameraModel, viewVector, ballRadius, ballCenter, &ballCenterPixel, &bordersPixel);
  ...
  *maxRadiusImg = 0;
  for (size_t i = 0; i < bordersPixel.size(); i++)
  {
    ...
    if (borderDistPx < *minRadiusImg)
      *minRadiusImg = borderDistPx;
    if (borderDistPx > *maxRadiusImg)
      *maxRadiusImg = borderDistPx;
  }
  return true;
}

Eigen::Vector2d HumanoidModel::cameraViewVectorToPanTilt(const Eigen::Vector3d& viewVector)
{
  ...
  double pitch = atan2(-viewInSelf.z(), sqrt(viewInSelf.x() * viewInSelf.x() + viewInSelf.y() * viewInSelf.y()));

  return Eigen::Vector2d(yaw, pitch);
}

Eigen::Vector2d HumanoidModel::cameraPixelToPanTilt(const CameraModel& cameraModel, const Eigen::Vector2d& pixel,
...
bool HumanoidModel::cameraWorldToPixel(const CameraModel& cameraModel, const Eigen::Vector3d& pos,
...
bool HumanoidModel::cameraPanTiltToPixel(const CameraModel& cameraModel, const Eigen::Vector2d& anglesPanTilt,
                                         Eigen::Vector2d& pixel)
{
  // Optical center
  ...
  Eigen::Matrix3d rot = Eigen::AngleAxisd(anglesPanTilt(0), Eigen::Vector3d::UnitZ()).toRotationMatrix() *
...
bool HumanoidModel::cameraLookAt(const Eigen::Vector3d& posTarget)
{
  double panDOF;
  double tiltDOF;
...
bool HumanoidModel::cameraLookAtNoUpdate(double& panDOF, double& tiltDOF, const Eigen::Vector3d& posTarget)
{
  ...
  Eigen::Vector3d targetInBase = Model::position("origin", "head_yaw", posTarget);
...
double HumanoidModel::cameraScreenHorizon(const CameraModel& cameraModel, double screenPosWidth)
{
...
LegIK::Vector3D HumanoidModel::buildTargetPos(const std::string& frame, const Eigen::Vector3d& footPos, bool isLeftLeg)
{
  ...
  {
    target = Model::position(frame, "trunk", footPos);
    if (isLeftLeg)
    {
      target -= _trunkToHipLeft;
    }
...
LegIK::Frame3D HumanoidModel::buildTargetOrientation(const std::string& frame, const Eigen::Matrix3d& rotation)
{
...
void HumanoidModel::setIKResult(const LegIK::Position& result, bool isLeftLeg)
{
  std::string prefix;
  if (isLeftLeg)
  {
...
void HumanoidModel::checkNaN(const LegIK::Position& result, const LegIK::Vector3D& pos,
                             const LegIK::Frame3D& orientation) const
{
  // Check if Nan is returned
  if (std::isnan(result.theta[0]) || std::isnan(result.theta[1]) || std::isnan(result.theta[2]) ||
      std::isnan(result.theta[3]) || std::isnan(result.theta[4]) || std::isnan(result.theta[5]))
  {
    throw std::logic_error("LegIK NaN invalid result. " + std::string("theta0=") + std::to_string(result.theta[0]) +
                           std::string(" ") + std::string("theta1=") + std::to_string(result.theta[1]) +
                           std::string(" ") + std::string("theta2=") + std::to_string(result.theta[2]) +
                           std::string(" ") + std::string("theta3=") + std::to_string(result.theta[3]) +
                           std::string(" ") + std::string("theta4=") + std::to_string(result.theta[4]) +
                           std::string(" ") + std::string("theta5=") + std::to_string(result.theta[5]) +
                           std::string(" pos=") + pos.pp() + std::string(" orientation=") + orientation.pp());
  }
}
